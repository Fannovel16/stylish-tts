...
def plot_mel_signed_difference_to_figure(
    ...
    *,
    ...
    if confidence_mask is not None:
        try:
            ...
            if mask.ndim == 2:
                ...
                overlay = np.broadcast_to(1.0 - np.clip(mask, 0.0, 1.0), diff.shape)
            ax_main.imshow(
                overlay,
                aspect="auto",
                origin="lower",
    ...
    if include_aggregates:
        frame_axis = (
            np.arange(per_frame_abs.shape[0]) if per_frame_abs.size else np.array([])
        )
        if frame_axis.size:
            ax_time.plot(frame_axis, per_frame_pos, color="#d62728")
            ax_time.plot(frame_axis, per_frame_neg, color="#1f77b4")
            ax_time.fill_between(
                frame_axis, 0, per_frame_pos, color="#d62728", alpha=0.15
            )
            ax_time.fill_between(
                frame_axis, 0, per_frame_neg, color="#1f77b4", alpha=0.15
            )
...
def plot_residual_temporal_grid(
    ...
    *,
    ...
    for idx, (ax, window) in enumerate(zip(axes, windows)):
        ...
        ax.set_title(
            f"Frames {idx * window_stride}-{idx * window_stride + window.shape[1]}"
        )
        ax.set_xlabel("Frames")
        if idx == 0:
            ax.set_ylabel("Mel Channels")
